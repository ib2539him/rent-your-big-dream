...
// Webfonts task
gulp.task("webfonts", () => {
  return gulp.src("assets/fonts/**/*.{ttf,woff,woff2,eot,svg}").pipe(gulp.dest("docs/fonts"));
});
 
// Copy other images (SVG, GIF,WEBP) to docs folder
gulp.task("images", () => {
  return gulp.src("assets/image/**/*.{svg,gif,webp,png,jpg,jpeg}").pipe(gulp.dest("docs/image"));
});
...
gulp.task("watch", () => {
  ...
  gulp.watch("assets/scss/web/*", gulp.series("webfonts"));
  gulp.watch("assets/image/**/*.{svg,gif}", gulp.series("images"));
